...
import { getHotelList } from './hotelapi';
    ...
    for (const section of sections) {
        for (const item of Array.isArray(section?.items) ? section.items : []) {
            ...
            entries.push({
                ...
                categoryName: label,
                categorySlug: generatedSlug,
                href: generatedSlug ? `/${encodeURIComponent(toSlug(regionSlug))}/${encodeURIComponent(generatedSlug)}` : ''
            });
        }
    }
...
export function buildCategorySidebarSections(
    ...
    return sections.map((section) => ({
        ...
        items: (section.items || []).map((item) => {
            ...
            const baseHref = `/${encodeURIComponent(normalizedCitySlug)}/${encodeURIComponent(categorySlug)}`;
...
export async function resolveCityContextFromSlug(citySlug = '') {
    ...
    try {
        const response = await getHotelList(normalizedCitySlug, 1, 1);
        const hotelsArray = response?.hotels || [];
        const firstHotel = hotelsArray[0];
        const cityId = response?.cityId ?? firstHotel?.cityId;

        if (cityId !== null && cityId !== undefined && cityId !== '') {
            resolvedContext = {
                cityId: Number(cityId),
                cityName:
                    String(firstHotel?.cityName ?? firstHotel?.city ?? formatCityName(normalizedCitySlug)).trim() ||
                    formatCityName(normalizedCitySlug)
            };
        }
    } catch (error) {
...
export async function resolveRegionContextFromSlug(regionSlug = '') {
    const normalizedRegionSlug = toSlug(regionSlug);
    if (!normalizedRegionSlug) return null;

    try {
        const response = await resolveSlug(`/${normalizedRegionSlug}`);
...
export async function resolveCategoryFromRegionSlug(categorySlug = '', regionSlug = '') {
    const normalizedRegionSlug = toSlug(regionSlug);
    const normalizedCategorySlug = toSlug(categorySlugFromAny(categorySlug));
    ...
    const regionContext = await resolveRegionContextFromSlug(normalizedRegionSlug);
